...
def cacheresponse(func, keyname=None):
    keyname = keyname or func.__name__

    @wraps(func)
    def inner(this, *args, **kwargs):
        prefix_keyname = '%s:%s:%s' % (this.__class__.__name__, this.pk, keyname)
        res = cache.get(prefix_keyname)

        if res is None:
            res = func(this, *args, **kwargs)
            cache.set(prefix_keyname, res)
        return res
